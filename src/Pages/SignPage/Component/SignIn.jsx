...
function SignIn() {
  ...
  const handleSignIn = async () => {
    if (!email || !password) {
      toast.error("Please fill in both email and password.", { containerId: "sign-container" });
      return;
    }
    ...
    try {
      ...
      if (user.isDefined) {
        ...
        toast.success("Sign-in successful!", { containerId: "sign-container" });
        setTimeout(() => {
          navigate("/home");
        }, 3500);
      }else{
        toast.error("Sorry, You account are reported. You can not signed from this email", { containerId: "sign-container" });
      }
      ...
      toast.error("An error occurred during sign-in. Please try again.", { containerId: "sign-container" });
    }
  };
              ...
              <GoogleLogin
                onSuccess={async (credentialResponse) => {
                  try {
                    ...
                    toast.success("Google sign-in successful!", { containerId: "sign-container" });
                    navigate("/home");
                  } catch (error) {
                    console.error("Error during Google sign-in:", error);
                    toast.error("Google sign-in failed. Please try again.", { containerId: "sign-container" });
                  }
                }}
                onError={() => {
                  console.log("Google Login Failed");
                  toast.error("Google sign-in failed.", { containerId: "sign-container" });
                }}
                render={(renderProps) => (
                  <button
              ...
              <FacebookLogin
                ...
                callback={async (response) => {
                  try {
                    ...
                    localStorage.setItem("userId", res.data.user_id);
                    toast.success("Facebook sign-in successful!", { containerId: "sign-container" });
                    navigate("/home");
                  } catch (error) {
                    console.error("Facebook login failed:", error);
                    toast.error("Facebook sign-in failed. Please try again.", { containerId: "sign-container" });
                  }
                }}
                render={(renderProps) => (
                  <button
