...
// Mock data for AI results (keeping as fallback)
const mockItems = [
  {
    id: "AI-001",
...
const AIDesignPage = () => {
  ...
  const [items, setItems] = useState(mockItems) // State to store API results
  ...
  const handleSubmit = async (e) => {
    ...
    if (inputValue.trim()) {
      try {
        const response = await fetch('http://localhost:8000/recomanded/ai', {
          method: 'POST',
          headers: {
        ...
        setItems(data.results || mockItems)
